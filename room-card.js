...
return html`<label class="ed-label">${label}</label> <input class="ed-input" type="text" .value="${value || ""}" placeholder="${placeholder || ""}" @input="${(e) => onChange(e.target.value)}" />`;
}

_toggle(label, value, onChange) {
return html`<div class="toggle-row"> <span class="ed-label">${label}</span> <label class="toggle-wrap"> <input type="checkbox" ?checked="${value}" @change="${(e) => onChange(e.target.checked)}" /> <span class="toggle-slider"></span> </label> </div>`;
}

_numSelect(label, value, options, onChange) {
return html` <div class="toggle-row"> <span class="ed-label">${label}</span> <select class="ed-select inline-select" .value="${String(value || 1)}" @change="${(e) => onChange(parseInt(e.target.value))}"> ${options.map((o) => html`<option value=”${o}” ?selected=”${o === (value || 1)}”>${o}</option>`)} </select> </div> `;
}

_colorPick(label, value, onChange) {
return html`<div class="color-row"> <span class="ed-label">${label}</span> <input type="color" class="color-picker" .value="${value || "#fbbf24"}" @input="${(e) => onChange(e.target.value)}" /> </div>`;
}
...
_tabGeneral() {
const cfg = this._config;
return html` <div class="section"> <div class="section-title">Room Identity</div> ${this._txt("Room Name", cfg.room_name, (v) => this._set("room_name", v), "e.g. Living Room")} ${this._txt("Room Icon (mdi:...)", cfg.room_icon, (v) => this._set("room_icon", v), "e.g. mdi:sofa")} </div> <div class="section"> <div class="section-title">Header Options</div> ${this._toggle("Show Date & Time", cfg.show_datetime, (v) => this._set("show_datetime", v))} ${this._toggle("Show Status Dot", cfg.show_status_dot, (v) => this._set("show_status_dot", v))} ${cfg.show_status_dot ? html`
...
` : ""} </div> <div class="section"> <div class="section-title">Camera</div> ${this._toggle("Show Camera Feed", cfg.show_camera, (v) => this._set("show_camera", v))} ${cfg.show_camera ? html`
...
return html` <div class="section"> <div class="section-title">Climate / Environment Sensors</div> <p class="hint"> Displayed side-by-side. Gauge color is <strong>automatic</strong> based on sensor type — temperature uses a severity gradient (blue→green→yellow→red), humidity is fixed blue. No color configuration needed. </p> ${items.map((s, i) => html`
<div class="list-item">
...
`)} <button class="btn-add" @click="${() => this._addItem("climate_sensors", { entity: "", label: "", unit: "", min: 0, max: 50 })}">+ Add Climate Sensor</button> </div> `;
}
...
return html`<div class="section"> <div class="section-title">Layout</div> ${this._numSelect("Sensor Columns", snCols, [1, 2, 3, 4], (v) => this._set("sensor_columns", v))} <p class="hint">${snCols === 1 ? "Row mode: full-width rows with name, timestamp and state." :`Grid mode: ${snCols}-column compact tiles.`}</p> </div> <div class="section"> <div class="section-title">Binary / State Sensors</div> <p class="hint"> Motion and presence entities are auto-detected and show the 🚶 icon with a pulsating glow when active. All other sensors use the configured MDI icon. </p> ${items.map((s, i) => { const stateMap     = s.state_map || {}; const stateEntries = Object.entries(stateMap); return html`
<div class="list-item">
<div class="list-item-header">
<span class="list-item-num">Sensor ${i + 1}</span>
...
${stateEntries.map(([state, disp]) => html`<div class="state-map-row"> <input class="ed-input sm" type="text" placeholder="state" .value="${state}" @input="${(e) => { const nm = { ...stateMap }; delete nm[state]; nm[e.target.value] = disp; this._updateItem("binary_sensors", i, "state_map", nm); }}" /> <span class="sm-arrow">→</span> <input class="ed-input sm" type="text" placeholder="label" .value="${disp.label || ""}" @input="${(e) => { this._updateItem("binary_sensors", i, "state_map", { ...stateMap, [state]: { ...disp, label: e.target.value } }); }}" /> <input type="color" class="color-picker sm" .value="${disp.color || "#fbbf24"}" @input="${(e) => { this._updateItem("binary_sensors", i, "state_map", { ...stateMap, [state]: { ...disp, color: e.target.value } }); }}" /> <button class="btn-remove-sm" @click="${() => { const nm = { ...stateMap }; delete nm[state]; this._updateItem("binary_sensors", i, "state_map", nm); }}">✕</button> </div>`)}
<button class=“btn-add sm” @click=”${() => {
this._updateItem(“binary_sensors”, i, “state_map”,
{ …stateMap, new_state: { label: “Label”, color: “#fbbf24” } });
}}”>+ Add State</button>
</div>
`; })} <button class="btn-add" @click="${() => this._addItem("binary_sensors", { entity: "", label: "", icon: "mdi:toggle-switch", state_map: { on:  { label: "On",  color: "#fbbf24" }, off: { label: "Off", color: "rgba(255,255,255,0.4)" }, } })}">+ Add Sensor</button> </div> `;
}
...
return html` <div class="section"> <div class="section-title">Switches / Lights / Controls</div> <p class="hint">Tappable tiles — tap to toggle. Columns auto-fit based on count.</p> ${items.map((s, i) => html`
<div class="list-item">
<div class="list-item-header">
...
`)} <button class="btn-add" @click="${() => this._addItem("switches", { entity: "", label: "", icon: "mdi:lightbulb", color: "#fbbf24" })}">+ Add Switch / Light</button> </div> `;
}
...
_tabPower() {
const cfg = this._config;
return html` <div class="section"> <div class="section-title">Power Display</div> <p class="hint">Arc gauge showing current wattage. Color shifts green→yellow→orange→red as load increases.</p> ${this._toggle("Show Power Gauge", cfg.show_power, (v) => this._set("show_power", v))} ${cfg.show_power ? html`
...
_tabMower() {
const cfg = this._config;
return html` <div class="section"> <div class="section-title">Husqvarna Automower</div> <p class="hint"> Displays state badge, battery bar and Start / Pause / Return to Dock controls. The icon shows the accurate Automower shape with spinning wheels and blade when mowing. </p> ${this._toggle("Show Mower Widget", cfg.show_mower, (v) => this._set("show_mower", v))} ${cfg.show_mower ? html`
...
return html` <div class="editor-root"> <div class="tab-bar"> ${tabs.map((t) => html`
...
return html`<div class="stream-wrap"> <ha-camera-stream allow-exoplayer muted playsinline ></ha-camera-stream> <div class="stream-overlay"> <span class="stream-label">${(this.label || "").toUpperCase()}</span> <span class="stream-live">● LIVE</span> </div> </div>`;
}

static get styles() {
return css`:host { display: block; } .stream-wrap { position: relative; border-radius: 14px; overflow: hidden; background: #0a0e1a; border: 1px solid rgba(255,255,255,0.08); } ha-camera-stream { width: 100%; display: block; max-height: 350px; object-fit: cover; --video-border-radius: 0; } .stream-overlay { position: absolute; bottom: 0; left: 0; right: 0; padding: 8px 12px; background: linear-gradient(transparent, rgba(0,0,0,0.6)); display: flex; justify-content: space-between; align-items: flex-end; } .stream-label { font-size: 10px; letter-spacing: 1px; color: rgba(255,255,255,0.5); text-transform: uppercase; } .stream-live  { font-size: 9px;  letter-spacing: 1px; color: #f87171; font-weight: 600; border: 1px solid rgba(248,113,113,0.4); padding: 2px 6px; border-radius: 4px; }`;
}
}
